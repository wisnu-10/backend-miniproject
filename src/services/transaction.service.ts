...
export const createTransaction = async (data: CreateTransactionInput) => {
    ...
    if (ticketTypes.length !== ticketTypeIds.length) {
        throw new Error("One or more ticket types not found or don't belong to this event");
    }
    ...
    if (data.promotion_code) {
        ...
        if (promotion.discount_percentage) {
            const percentDiscount = totalAmount.mul(promotion.discount_percentage).div(100);
    ...
    if (data.coupon_code) {
        ...
        if (coupon.discount_percentage) {
            const percentDiscount = totalAmount.mul(coupon.discount_percentage).div(100);
            discountAmount = discountAmount.add(percentDiscount);
    ...
    const transaction = await prisma.$transaction(async (tx) => {
        ...
        const totalTickets = itemsWithDetails.reduce((sum, item) => sum + item.quantity, 0);
        ...
        if (pointsUsed > 0) {
            ...
            for (const point of availablePoints) {
                ...
                const deductFromThis = Math.min(point.remaining_amount, remainingToDeduct);
    ...
    return {
        ...
        time_remaining_seconds: Math.max(0, Math.floor((paymentDeadline.getTime() - Date.now()) / 1000)),
    };
};
...
export const uploadPaymentProof = async (
    ...
    if (transaction.status !== TransactionStatus.WAITING_PAYMENT) {
        throw new Error(`Cannot upload payment proof. Current status: ${transaction.status}`);
    }
...
export const cancelTransaction = async (transactionId: string, userId: string) => {
    ...
    if (transaction.status !== TransactionStatus.WAITING_PAYMENT) {
        throw new Error("Can only cancel transactions that are waiting for payment");
    }
...
export const updateTransactionStatus = async (
    ...
    if (transaction.status !== TransactionStatus.WAITING_CONFIRMATION) {
        throw new Error("Can only accept/reject transactions that are waiting for confirmation");
    }
    ...
    const updated = await prisma.transaction.update({
        where: { id: transactionId },
        data: {
            status: status === "DONE" ? TransactionStatus.DONE : TransactionStatus.REJECTED,
        },
        include: {
            items: {
                include: {
...
export const getTransactionById = async (transactionId: string, userId: string) => {
    ...
    if (transaction.status === TransactionStatus.WAITING_PAYMENT) {
        timeRemaining = Math.max(
            0,
            Math.floor((transaction.payment_deadline.getTime() - now.getTime()) / 1000)
        );
    }
